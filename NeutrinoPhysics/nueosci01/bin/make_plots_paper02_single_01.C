...
void makePlots() 
{
  ...
  makePlots("Vacuum","ModelA", "Pee","detection.root");
...
void makePlots( const char * model, const char * src, const char * prob, const char * infile) 
{
  ...
  TString dataPxx = TString( "Ratio_" ) +
    TString( model ) + TString("_") + TString( src ) + TString("_") + TString(prob) +  TString("/data");
  TString dataAPxx =TString( "Ratio_" ) +
    TString( model ) + TString("_") + TString( src ) + TString("_a") + TString(prob) +  TString("/data");
  ...
  TString cname = TString(src) + TString("_") + TString(prob);
  
  TCanvas * c1 = new TCanvas( cname.Data(), "track/shower ratio", 184,60,861,263); 
  ...
  PxxTreeANu->SetBranchAddress("Ratio",&yy);
  ...
  ProbNu[0]->SetFillColor(10);
  ProbNu[0]->SetMaximum(5.0);
  ProbNu[0]->SetMinimum(1.0);

  TString yaxis = ((TObjString*)v_Labels->At(0))->GetString();
  ProbNu[0]->GetYaxis()->SetTitle( "ratio" );
  ...
  saveAs.str("");
  saveAs << path << src << "/pdf/" << "ratio_" << src << "_" << prob << "_nuanu" << ".pdf";
  c1->SaveAs( saveAs.str().c_str() );
  
  saveAs.str("");
  saveAs << path << src << "/png/" << "ratio_" << src << "_" << prob << "_nuanu" << ".png";
  c1->SaveAs( saveAs.str().c_str() );

  saveAs.str("");
  saveAs << path << src << "/eps/" << "ratio_" << src << "_" << prob << "_nuanu" << ".eps";
  c1->SaveAs( saveAs.str().c_str() );
